...
function RhythmRuler () {
    ...
    this._calculateZebraStripes = function(rulerno) {
		...
		console.log("_calculateZebraStripes");
    ...
    this._dissectRuler = function (event) {
		console.log("_dissectRuler");
        var inputNum = docById('dissectNumber').value;
        if (isNaN(inputNum)) {
            inputNum = 2;
        } else {											// Easter Egg
            inputNum = Math.abs(Math.floor(inputNum));		// Easter Egg
        }													// Easter Egg

        docById('dissectNumber').value = inputNum;

        var cell = event.target;							// target is a cell
        this._rulerSelected = cell.parentNode.id[5];
        this.__dissect(cell, inputNum);
    };

    this.__dissect = function (cell, inputNum) {
		console.log("__dissect");
        var that = this;

        var ruler = docById('ruler' + this._rulerSelected);	// is ruler the main one that gets displayed?
        var newCellIndex = cell.cellIndex;					// take index from what? From the cell clicked?
        var noteValues = this.Rulers[this._rulerSelected][0]; // Array noteValues <--- ?
        var divisionHistory = this.Rulers[this._rulerSelected][1]; // divisionHistory and ruler difference: ?
		
		for (var i=0;i<this.Rulers[this._rulerSelected].length;i++) {
			console.log("___" + "Rulers[...][" + i + "]= " + this.Rulers[this._rulerSelected][i]);
		}
    ...
    this.init = function(logo) {
        console.log('init RhythmRuler function');
        this._logo = logo;
		...
		docById('recordRhythm').style.display = 'inline';		//drumDiv defined in HTML
